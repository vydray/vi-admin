...
export default function TwitterSettingsPage() {
  ...
  // 投稿スロット (よく使う時刻) 編集用 state
  const [slotTimes, setSlotTimes] = useState<string[]>([])
  const [newSlotTime, setNewSlotTime] = useState('')
  const [savingSlots, setSavingSlots] = useState(false)

  // 設定読み込み時に slotTimes も同期
  useEffect(() => {
    setSlotTimes(settings?.default_post_times ?? [])
  }, [settings])
  ...
  const addSlot = () => {
    ...
    if (slotTimes.includes(v)) {
      toast.error('同じ時刻が既に登録されています')
      return
    }
    setSlotTimes([...slotTimes, v].sort())
    setNewSlotTime('')
  }
  ...
  const saveSlots = async () => {
    ...
    try {
      ...
      setSlotTimes(data.default_post_times ?? [])
      // 設定 state も同期
      setSettings(prev => prev ? { ...prev, default_post_times: data.default_post_times ?? [] } : prev)
    } catch {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.content}>
        ...
        {/* 投稿スロット (よく使う時刻) */}
        <div style={styles.section}>
          <h2 style={styles.sectionTitle}>投稿スロット（よく使う時刻）</h2>
          <p style={styles.description}>
