  ...
  return (
    <div style={{ padding: isMobile ? '56px 12px 24px' : '24px' }}>
      ...
      {loading ? (
        ...
        <MobileDailyView key={yearMonth} data={data} events={events} openTargetEdit={openTargetEdit} />
      ) : (
...
function MobileDailyView({
  ...
  // 初期選択日: 当月なら今日、そうでなければ最後に売上のあった日
  const [selIdx, setSelIdx] = useState(() => {
    const t = new Date()
    const ym = `${t.getFullYear()}-${String(t.getMonth() + 1).padStart(2, '0')}`
    if (ym === data.yearMonth) return Math.min(t.getDate() - 1, rows.length - 1)
    for (let i = rows.length - 1; i >= 0; i--) if (rows[i].sales > 0) return i
    return 0
  })
  const idx = Math.min(Math.max(selIdx, 0), rows.length - 1)
  const r = rows[idx]
  ...
  const dow = dowOf(r.date)
  const dColor = dow === 0 || isHolidayDate(r.date) ? '#dc2626' : dow === 6 ? '#2563eb' : '#1f2937'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {/* 月サマリー */}
      <div style={mCard}>
        <div style={{ fontSize: '13px', color: '#64748b', fontWeight: 700, marginBottom: '10px' }}>{data.yearMonth} 月サマリー</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          <DStat label="総売上" value={yen(s.totalSales)} big />
          <DStat label="売上目標" value={s.targetTotal > 0 ? yen(s.targetTotal) : '—'} big />
          <DStat label="粗利" value={yen(s.grossProfit)} color={s.grossProfit < 0 ? '#dc2626' : '#15803d'} big />
          <DStat label="売上達成率" value={achv != null ? (achv * 100).toFixed(1) + '%' : '—'} color={barColor} big />
        </div>
        <div style={{ height: '8px', background: '#f1f5f9', borderRadius: '4px', overflow: 'hidden', margin: '10px 0' }}>
          <div style={{ height: '100%', width: `${achv != null ? Math.min(achv * 100, 100) : 0}%`, background: barColor, transition: 'width .3s ease' }} />
        </div>
      ...
      {/* 日付セレクタ（横スクロール） */}
      <div style={{ display: 'flex', overflowX: 'auto', gap: '6px', padding: '2px', WebkitOverflowScrolling: 'touch' }}>
        {rows.map((row, i) => {
          const d = dowOf(row.date)
          const sel = i === idx
          const c = d === 0 || isHolidayDate(row.date) ? '#dc2626' : d === 6 ? '#2563eb' : '#475569'
          return (
            <button
              key={row.date}
              onClick={() => setSelIdx(i)}
              style={{
                flex: '0 0 auto', width: '42px', padding: '6px 0', borderRadius: '8px', cursor: 'pointer',
                border: sel ? '2px solid #3b82f6' : '1px solid #e2e8f0',
                background: sel ? '#eff6ff' : '#fff', color: sel ? '#1d4ed8' : c,
                fontWeight: sel ? 700 : 500, lineHeight: 1.25,
              }}
            >
              <div style={{ fontSize: '14px' }}>{row.day}</div>
              <div style={{ fontSize: '9px' }}>{WD[d]}</div>
            </button>
          )
        })}
      </div>
      ...
      <div style={mCard}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '4px' }}>
          <div style={{ fontSize: '17px', fontWeight: 700, color: dColor }}>{r.day}日（{WD[dow]}）</div>
          {ev && <span style={{ color: '#7c3aed', fontSize: '12px' }}>{ev}</span>}
        </div>

        <DSection title="売上">
          <div style={dGrid}>
            <DStat label="店舗売上" value={yen(r.sales)} />
            <DStat label="現金" value={yen(r.cashSales)} />
            <DStat label="カード" value={yen(r.cardSales)} />
            <DStat label="その他" value={yen(r.otherSales)} />
            <DStat label="BASE" value={yen(r.baseSales)} />
            <DStat label="総売上" value={yen(r.totalSales)} />
          </div>
          <DTargetRow label="売上目標" value={r.target} metric="sales" achv={r.achievementRate} onEdit={() => openTargetEdit(r.date, 'sales', r.target)} />
        </DSection>

        <DSection title="出勤">
          <div style={dGrid}>
            <DStat label="シフト" value={`${r.shiftCount}人`} />
            <DStat label="出勤" value={`${r.attendanceCount}人`} />
            <DStat label="出勤率" value={pct(r.attendanceRate)} />
          </div>
          <DTargetRow label="目標出勤" value={r.targetAttendance} metric="attendance" achv={r.achievementRateAttendance} onEdit={() => openTargetEdit(r.date, 'attendance', r.targetAttendance)} />
        </DSection>

        <DSection title="来客">
          <div style={dGrid}>
            <DStat label="会計数" value={`${r.orderCount}`} />
            <DStat label="来店人数" value={`${r.guests}人`} />
            <DStat label="客単価" value={yen(r.avgSpend)} />
            <DStat label="初回" value={`${r.firstTimeGuests}`} />
            <DStat label="再訪" value={`${r.returnGuests}`} />
            <DStat label="常連" value={`${r.regularGuests}`} />
            <DStat label="LINE予定" value={`${r.lineReservedGuests}`} />
          </div>
          <DTargetRow label="目標来客" value={r.targetGuests} metric="guests" achv={r.achievementRateGuests} onEdit={() => openTargetEdit(r.date, 'guests', r.targetGuests)} />
        </DSection>

        <DSection title="利益">
          <div style={dGrid}>
            <DStat label="人件費" value={yen(r.laborCost)} />
            <DStat label="人件費率" value={pct(r.laborCostRate)} />
            <DStat label="経費" value={yen(r.expense)} />
            <DStat label="経費率" value={pct(r.expenseRate)} />
            <DStat label="粗利" value={yen(r.grossProfit)} color={r.grossProfit < 0 ? '#dc2626' : '#15803d'} />
          </div>
        </DSection>
      </div>
    </div>
...
const dGrid: React.CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px 10px', marginBottom: '8px' }

function DStat({ label, value, color = '#1f2937', big = false }: { label: string; value: string; color?: string; big?: boolean }) {
  return (
    <div>
      <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '2px' }}>{label}</div>
      <div style={{ fontSize: big ? '16px' : '14px', fontWeight: 700, color }}>{value}</div>
    </div>
  )
}
...
function DSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ borderTop: '1px solid #eef2f7', paddingTop: '8px', marginTop: '8px' }}>
...
function DTargetRow({ label, value, metric, achv, onEdit }: {
  label: string
  value: number | null
  metric: TargetMetric
  achv: number | null
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f8fafc', borderRadius: '6px', padding: '7px 10px', marginTop: '2px' }}>
      <span style={{ fontSize: '12px', color: '#64748b', fontWeight: 600 }}>{label}</span>
      <span style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        {targetCell(value, metric, onEdit)}
        <span style={{ fontSize: '13px', minWidth: '46px', textAlign: 'right' }}>{achievementDisplay(achv)}</span>
      </span>
    </div>
  )
}
